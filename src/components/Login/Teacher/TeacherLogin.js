...
function Teacherlogin() {
  ...
  const onLogin = (event) =>{
    ...
    setDisable(true)
    axios.post('http://localhost:3001/vendor/vendorlogin',data)
      .then(function (response) {
      console.log(response.data.data)
      console.log(response.data.token)
      localStorage.setItem("teacherid",response.data.token);
    ...
    return (
        <>
        <div className='fo'>
        <h1 style={{color:'black'}}>Vandor Login</h1>
       ...
       <sapn>Create one ||</sapn>   <Link to='/teacher/registration' className='signup' style={{color:'red'}} > Sign Up</Link><br/>
